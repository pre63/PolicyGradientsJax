...
def compute_vmpo_loss(
  ...
  if discrete:
    ...
    kl_loss_sigma = jnp.mean(
      params.alpha_sigma * (eps_alpha_sigma - jax.lax.stop_gradient(kl_div_sigma)) + jax.lax.stop_gradient(params.alpha_sigma) * kl_div_sigma
    )
  ...
  log_mean_scaled_adv = jax.scipy.special.logsumexp(
    jax.lax.stop_gradient(advantages) / params.eta,
    b=jax.lax.stop_gradient(1 / len_advantages),
  )
...
def main(_):
  ...
  def gradient_update_fn(
    ...
    def f(*args, optimizer_state):
      ...
      params = params.replace(
        alpha=jnp.maximum(params.alpha, min_lagrange),
  ...
  gradient_update_fn = gradient_update_fn(
    ...
    has_aux=True,
    min_lagrange=Config.min_lagrange,
  ...
  def minibatch_step(
    ...
    (_, metrics), params, optimizer_state = gradient_update_fn(params, data, key_loss, optimizer_state=optimizer_state)
