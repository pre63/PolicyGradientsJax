...
from .envs import Transition, has_discrete_action_space, is_atari_env, make_env
from .networks.distributions import (DiscreteDistribution,
                                     NormalTanhDistribution,
                                     ParametricDistribution,
                                     PolicyNormalDistribution)
from .networks.networks import (ActivationFn, FeedForwardNetwork,
                                make_atari_feature_extractor,
                                make_policy_network, make_value_network)
...
def make_inference_fn(agent_networks: Union[Networks, AtariNetworks]):
  ...
  def make_policy(params: Any, deterministic: bool = False) -> Policy:
    ...
    @jax.jit
    def policy(observations: jnp.ndarray, key_sample: jnp.ndarray) -> Tuple[jnp.ndarray, Mapping[str, Any]]:
...
def make_networks(
  ...
  if discrete_policy:
    ...
    parametric_action_distribution = PolicyNormalDistribution(event_size=action_size)
...
def compute_policy_objective(
  ...
  target_action_log_probs = parametric_action_distribution.log_prob(policy_logits, data.extras["policy_extras"]["raw_action"])
...
def compute_policy_objective_and_kl(
  ...
  """helper function to enable computing kl gradients.
    ...
    """
  (policy_objective, kl_div), policy_objective_grad = policy_objective_grad_fn(params, data, hidden, advantages, network)
...
def jacobian_vector_product(
  ...
  (_, (_, _)), grad_kl = policy_objective_and_kl_grad_fn(params, data, hidden, advantages, network)
...
def trpo_policy_update(
  ...
  if shared_feature_extractor:
    feature_extractor_apply = network.feature_extractor.apply
    hidden_boot = feature_extractor_apply(params.feature_extractor, data.next_observation[-1])
  ...
  (kl_div, (policy_objective, policy_objective_grad)), grad_kl = policy_objective_and_kl_grad_fn(params, data, hidden, advantages, network)
  ...
  def line_search_step(carry):
    ...
    logits = policy_apply(new_params.policy, hidden)
    target_action_log_probs = parametric_action_distribution.log_prob(logits, data.extras["policy_extras"]["raw_action"])
  ...
  (
    ...
    line_search_step,
    (0, policy_objective, 100.0, line_search_backtrack_coeff, params),
  )
...
def main(_):
  ...
  env_step_per_training_step = Config.batch_size * Config.unroll_length * Config.num_minibatches
  ...
  compute_policy_objective_and_kl_fn = partial(
    compute_policy_objective_and_kl,
    policy_objective_grad_fn=policy_objective_grad_fn,
  )
  ...
  gradient_update_fn = gradient_update_fn(v_loss_fn, optimizer, pmap_axis_name=_PMAP_AXIS_NAME, has_aux=True)

  # minibatch training step for critic
  def v_minibatch_step(
    carry,
    data: Transition,
  ):
    ...
    if process_id == 0 and Config.eval_env and training_step % Config.eval_every == 0:
      ...
      scores.append(
        (
          ...
          np.mean(eval_ep_lengths),
          metrics["training/kl_div"],
        )
      )
